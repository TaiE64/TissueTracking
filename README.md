...
### 5-seq head-to-head (n=26 pts) — 4 个 setup 比较

| setup | acc@4 | acc@8 | acc@16 | acc@32 | acc@64 | **avg acc** | median (px) |
|---|---|---|---|---|---|---|---|
| baseline RGB-only | 0.269 | 0.462 | 0.615 | 0.731 | 0.769 | **0.5692** | 9.67 |
| v1 (self-attn + distill anchor) | 0.269 | 0.423 | 0.615 | 0.769 | 0.769 | **0.5692** | 9.77 |
| v1 noAnchor (w_distill=0) | ≈v1 (training trajectory identical) | | | | | | |
| **crossAttn** (RGB queries MS) | 0.269 | 0.462 | 0.615 | 0.731 | **0.808** | **0.5769** | 10.08 |

### 训练 ablation 总结

| variant | photo loss EMA plateau | STIR avg acc (5 seq) | 结论 |
|---|---|---|---|
| v1 self-attn + distill anchor (half-res) | 0.0071 | 0.5692 | 同 baseline |
| noAnchor self-attn (w_distill=0) | 0.0071 | (training trajectory identical to v1) | 同 v1 |
| full-resolution v1 (~step 130) | 0.0030 (step 138) | (not eval'd) | 同 half-res 轨迹 |
| crossAttn (RGB query MS) | 0.0089 | 0.5769 | 略胜 baseline (+1.4%) on 5 seqs |

### 诊断:`bottleneck 不是架构,是 photometric loss + info ceiling`
- 架构变化(self-attn ↔ cross-attn): ±1%
- anchor 强弱(0.01 → 0): 完全相同 plateau
- 分辨率(half ↔ full): 完全相同 plateau
- photometric self-sup 不指向 "用 spectral 改善 warp"
- MST++ 输出是 RGB 函数 → 信息上限 = RGB
